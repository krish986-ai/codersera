...
export default async function handler(request: any, response: any) {
  ...
  const photo = typeof ticket.photoBase64 === 'string' ? ticket.photoBase64 : '';
  if (!ticket.gdprConsent || !String(ticket.fullName || '').trim() || !String(ticket.phoneNumber || '').trim()
    || !/^\S+@\S+\.\S+$/.test(email) || !photo || photo.length > 1.4 * 1024 * 1024
    || ticket.qrPayload !== `${ticket.id}|${email}|${rollNumber}|${ticket.eventId}`) {
  ...
  try {
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      if (!event.isActive) throw new Error('EVENT_CLOSED');
    ...
    const status = error.message === 'DUPLICATE' || error.message === 'SOLD_OUT' || error.message === 'EVENT_CLOSED' ? 409 : error.message === 'EVENT_NOT_FOUND' ? 404 : 503;
    response.status(status).json({ error: error.message === 'DUPLICATE' ? 'A ticket already exists for this attendee.' : error.message === 'SOLD_OUT' ? 'This event is sold out.' : error.message === 'EVENT_CLOSED' ? 'Registration for this event is closed.' : 'Registration is temporarily unavailable.' });
  }
}
