...
const ADMIN_TOKEN_STORAGE_KEY = 'codersera_admin_token';
...
export async function findExistingTicket(email: string, rollNumber: string, eventId: string) {
  try {
    const tickets = (await request(`/api/tickets/lookup?q=${encodeURIComponent(email.trim().toLowerCase())}`).catch(() => [])) as StudentTicket[];
    const rollMatches = (await request(`/api/tickets/lookup?q=${encodeURIComponent(rollNumber.trim().toUpperCase())}`).catch(() => [])) as StudentTicket[];
    const all = [...(Array.isArray(tickets) ? tickets : []), ...(Array.isArray(rollMatches) ? rollMatches : [])];
    return all.find(t => t && t.eventId === eventId) || null;
  } catch {
    return null;
  }
}
export async function findTicketsByEmail(query: string) {
  const q = String(query || '').trim();
  if (!q) return [];
  return request(`/api/tickets/lookup?q=${encodeURIComponent(q.toLowerCase())}`);
}
export async function addCommunityEvent(eventData: Omit<CommunityEvent, 'id'> & { id?: string }) {
  const event = { ...eventData, id: eventData.id || `evt-${crypto.randomUUID().slice(0, 12)}`, totalSeats: Number(eventData.totalSeats) || 100, availableSeats: Number(eventData.availableSeats) || Number(eventData.totalSeats) || 100, generatedPasses: eventData.generatedPasses ?? 0, isActive: eventData.isActive ?? true, isFeatured: eventData.isFeatured ?? false };
...
export async function uploadEventFeaturedImage(eventId: string, file: File) {
  if (!file.type.startsWith('image/')) throw new Error('Please select an image file.');
  if (file.size > 1.5 * 1024 * 1024) throw new Error('Featured images must be 1.5 MB or smaller.');
  ...
  const result = await request('/api/admin/event-image', json({ eventId, dataUrl }));
